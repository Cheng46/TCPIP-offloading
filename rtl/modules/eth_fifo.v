...
$Description :  This fifo queue is design for storing both ethernet frame and the size of each ethernet frame.
                ...
                which is 12144 bits, so we need 3036 lines for storing a ethernet frame.

                In this design, we will use FIFO_DEPTH = 12144 lines (data fifo queue), which means we can store at least 4 ethernet frames. 
                Also, we will use FIFO_DEPTH = 24 lines (len fifo queue), which means we can store 24 ethernet framess length. 
                (Because ethernet could be 64 bytes, 12144/512 = 23.7 => we need 24 lines)
******************************************************************************************************************************************************************/
...
>>>>>>> 51757a0 (WIP: save changes before rebase)
    ...
    output reg[        `MII_DATA_WIDTH-1:0]   o_data_fifo_data,      
